...
public sealed class PaymentTimingOptions
{
    ...
    public int GatewayAttemptLifetimeMinutes { get; init; } = 10;
    public int InstantCodeReservationLifetimeMinutes { get; init; } = 60;
    public int ManagedStockReservationLifetimeMinutes { get; init; } = 15;
...
public sealed class PaymentTimingOptionsValidator : IValidateOptions<PaymentTimingOptions>
{
    public ValidateOptionsResult Validate(string? name, PaymentTimingOptions options)
    {
        if (options.GatewayAttemptLifetimeMinutes <= 0 ||
            ...
            options.ManagedStockReservationLifetimeMinutes <= 0 ||
        ...
        if (options.GatewayAttemptLifetimeMinutes >= options.ManagedStockReservationLifetimeMinutes)
            return ValidateOptionsResult.Fail(
                "PaymentTiming:GatewayAttemptLifetimeMinutes must be less than ManagedStockReservationLifetimeMinutes so an expired attempt can safely recover inventory.");
