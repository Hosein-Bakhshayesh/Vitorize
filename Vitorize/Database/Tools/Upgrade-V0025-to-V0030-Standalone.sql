/*
  Vitorize standalone production upgrade: V0025 -> V0030
...
BEGIN TRY
    ...
    (
        SELECT 1
        FROM dbo.DatabaseScriptHistory
        WHERE ScriptVersion IN (N'V0026', N'V0027', N'V0028', N'V0029', N'V0030')
    )
        THROW 51323, N'حداقل یکی از نسخه‌های V0026 تا V0030 قبلاً ثبت شده است. برای جلوگیری از اجرای تکراری، تغییری اعمال نشد.', 1;
    ...
    IF (SELECT COUNT(*) FROM dbo.DatabaseScriptHistory WHERE ScriptVersion IN (N'V0026', N'V0027', N'V0028', N'V0029', N'V0030') AND Success = 1) <> 5
        THROW 51331, N'اعتبارسنجی نهایی ناموفق بود: تاریخچه کامل مهاجرت ثبت نشده است.', 1;

    COMMIT TRANSACTION;

    SELECT
        N'ارتقای Vitorize از V0025 به V0030 با موفقیت انجام شد.' AS Result,
