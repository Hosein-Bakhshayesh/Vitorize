...
test('product is present in initial HTML with canonical metadata and safe JSON-LD', async ({ request }) => {
  const response = await request.get('/product/e2e-seo-product');
  expect(response.status()).toBe(200);
  const html = decodeNumericEntities(await response.text());
  expect(html).toContain('محصول قطعی آزمون مرورگر');
  expect(html).toMatch(/<title>[^<]*محصول آزمون سئو[^<]*<\/title>/);
  expect(html).toMatch(/<meta name="description" content="توضیح متای قطعی برای آزمون سئو\."/);
