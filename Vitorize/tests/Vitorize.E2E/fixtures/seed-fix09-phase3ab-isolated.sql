...
SELECT PolicyId, N'FIX09-3AB-' + Code, N'FIX09 3AB ' + Code, 1, SYSUTCDATETIME() FROM @Scenarios;
INSERT dbo.KycPolicyVersions (Id, KycPolicyId, Version, Status, CustomerTitle, CustomerInstructions, CreatedAt, PublishedAt)
SELECT VersionId, PolicyId, 1, 2, N'FIX09 3AB ' + Code, Instructions, SYSUTCDATETIME(), SYSUTCDATETIME() FROM @Scenarios;
...
SELECT NEWID(), VersionId, @DocA, 1, 10,
    CASE WHEN Code = N'MULTI' THEN N'راهنمای مدرک الزامی - خط اول' + CHAR(10) + N'<strong>این متن باید عادی نمایش داده شود</strong>' END,
...
SELECT NEWID(), VersionId, @DocB, 0, 20, N'راهنمای مدرک اختیاری - خط اول' + CHAR(10) + N'خط دوم', 0 FROM @Scenarios WHERE Code = N'MULTI';
...
-- The current product points to V2, while the V1 order above keeps an immutable
-- optional snapshot.  Browser tests must read the item snapshot, never this row.
UPDATE dbo.Products SET KycPolicyVersionId = (SELECT VersionId FROM @Scenarios WHERE Code = N'V2') WHERE Id = @ProductId;
