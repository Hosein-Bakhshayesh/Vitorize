...
IF EXISTS (SELECT [Key] FROM dbo.Settings WHERE [Key] IN (N'StorefrontPersianFont',N'StorefrontEnglishFont') GROUP BY [Key] HAVING COUNT(*)<>1) OR (SELECT COUNT(*) FROM dbo.Settings WHERE [Key] IN (N'StorefrontPersianFont',N'StorefrontEnglishFont'))<>2 THROW 53116,'Required storefront typography settings are absent or duplicated.',1;
